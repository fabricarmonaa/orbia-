...
export function evaluatePassword(password: string, context: PasswordPolicyContext = {}): PasswordPolicyEvaluation {
  ...
  // Only hard requirements: min 6 chars, not common, not containing personal data
  const isValid = minLength && notCommon && notContainsDni && notContainsTenantCode && notContainsTenantName;

  const warnings: string[] = [];
  if (!minLength) warnings.push("Debe tener al menos 6 caracteres");
  ...
  // Score is informational only — complexity adds bonus points
  let score = 0;
