...
export async function changeOrderStatusWithHistory(params: { tenantId: number; orderId: number; statusCode: string; statusId?: number | null; changedById: number; note?: string | null }) {
  await db.transaction(async (tx) => {
    await tx
      .update(orders)
      .set({ statusCode: params.statusCode, statusId: params.statusId || null, updatedAt: new Date() })
