...
export async function aiGetJson(path: string, headers?: Record<string, string>) {
  ...
  if (!res.ok) {
    throw new AiClientError("AI_BAD_RESPONSE", `AI GET ${path} failed`, { status: res.status, body });
  }
  return body;
}
...
export async function aiPostForm(path: string, form: FormData, headers?: Record<string, string>) {
  ...
  if (!res.ok || body?.success === false) {
    throw new AiClientError("AI_BAD_RESPONSE", `AI POST ${path} failed`, { status: res.status, body });
  }
  return body;
}
