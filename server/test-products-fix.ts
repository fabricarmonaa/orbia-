...
async function test() {
  ...
  const all = await db.select().from(products).where(eq(products.tenantId, tenantId));
  console.log(`Total raw products in DB: ${all.length}`);

  console.log('--- API QUERY ---');
  const res = await queryProductsByFilters(tenantId, false, { status: 'all', page: 1, pageSize: 20 } as any);
