                ...
                <table className="w-full text-sm">
                  <thead className="bg-muted/60">
                    <tr>
                      ...
                      <th className="p-2 text-left">Dirección/Notas</th>
                    ...
                    {(importPreview.rowsPreview || []).slice(0, 10).map((row: any, idx: number) => {
                      ...
                      return (
                        <tr key={idx} className={`border-t ${hasErrors ? "bg-red-50/50" : "bg-green-50/20"}`}>
                          ...
                          <td className="p-2 font-medium">{row.normalized.name || <span className="text-red-500 text-xs">Falta</span>}</td>
                          <td className="p-2">{row.normalized.doc || "-"}</td>
                          <td className="p-2 text-xs">
                            {row.normalized.email && <div>{row.normalized.email}</div>}
                            {row.normalized.phone && <div>{row.normalized.phone}</div>}
                            {!row.normalized.email && !row.normalized.phone && "-"}
                          </td>
                          <td className="p-2 text-xs truncate max-w-[150px]">{row.normalized.address || row.normalized.notes || "-"}</td>
                          <td className="p-2">
                            {hasErrors ? (
