...
export default function PosPage() {
  ...
  async function submitSale() {
    ...
    try {
      ...
      // Handle 409 Conflict (e.g. INSUFFICIENT_STOCK) without crashing
      if (res.status === 409) {
        ...
        if (body?.code === "INSUFFICIENT_STOCK") {
          ...
          toast({
            title: "Conflicto al registrar venta",
            description: body?.error || "La venta fue rechazada por el servidor",
            variant: "destructive",
          });
        }
        return;
      ...
      if (!res.ok) {
        const body = await res.json().catch(() => ({} as any));
        throw new Error(body?.error || `Error ${res.status} al registrar la venta`);
      }
      ...
      toast({
        ...
        description: err?.message || "Error inesperado. Intentá nuevamente.",
        variant: "destructive",
      });
    }
  }
